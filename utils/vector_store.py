...
class VectorStore:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            self.client = MilvusClient(uri=f"http://{config.MILVUS_HOST}:{config.MILVUS_PORT}")      #主机地址:端口号

            # 2.3 embedding model的路径
            embedding_model_path = os.path.join(config.MODEL_PATH, config.EMBEDDING_MODEL)
                # 2.4 打印embedding模型路径以便调试
            print(f"embedding_model_path: {embedding_model_path}")

            # 加几行代码，在初始化模型前打印路径和文件列
            print("模型路径:", embedding_model_path)
            print("文件列表:", os.listdir(embedding_model_path))
            print("sentencepiece 文件是否存在:",
                  os.path.exists(os.path.join(embedding_model_path, "sentencepiece.bpe.model")))
            ...
            self.embedding_function = BGEM3EmbeddingFunction(model_name=embedding_model_path, device='cpu',
                                                           use_fp16=False)
                # 2.6 获取嵌入函数的稠密向量维度(Milvus创建时必须指定向量维度)
            self.dense_dim = self.embedding_function.dim["dense"]
            # print(dir(self.embedding_function))

            # 2.7 reranker model的路径
            ...
            self.mongo_client = MongoClient(mongo_uri, serverSelectionTimeoutMS=5000)   #  2.11 初始化MongoDB客户端,设置连接超时时间
                                                                                        # Milvus和MangoDB必须要初始化客户端,他们是远程服务,而BGEM3是本地模型,加载就是初始化了
            self.mongo_client.admin.command("ping")                                      # 2.12 测试MongoDB连接是否有效
            self.mongo_db = self.mongo_client[config.MONGO_DB]                           # 2.13 获取MongoDB数据库实例(从MongoDB服务器中找到[数据库名字],并返回一个实例)
            self.mongo_collection = self.mongo_db["resumes"]                             # 2.14 获取MongoDB简历集合


            # 2.15 初始化Elasticsearch客户端
            self.es_client = Elasticsearch(config.ES_HOST)
            if not self.es_client.indices.exists(index=config.ES_INDEX_NAME):            # 2.16 检查Elasticsearch索引是否存在,若不存在则创建
    ...
    def store_resume(self, doc: Document, chunks: List[Document], structured_data: Dict[str, Any]) -> bool:
        ...
        for idx, chunk in enumerate(chunks):
            # 4.10 获取稀疏向量的索引和数据
            sparse_indices = embeddings["sparse"].indices[              #embeddings["sparse"]就是声明是稀疏向量而不是稠密向量
            ...
            chunk_data ={
                ...
                "dense_vector":embeddings["dense_vector"][idx],
                "sparse_vector":sparse_vector,
                "doc_hash":doc_hash,
                "parent_content":chunk.metadata.get("parent_content",""),
                **structured_data
            }
            # 4.13 将数据添加到插入列表,存的是每个chunk的向量数据
    ...
    def hybrid_search_with_rerank(self, query: str, params: Dict[str, Any]) -> List[Document]:
        ...
        #得到query向量,encode_queries会得到两组向量,稠密和稀疏,所以要分别获得
        query_embeddings = self.embedding_function.encode_queries([query])
        #查询的稠密向量
        dense_vector = query_embeddings["dense"][0].tolist()
        #查询的稀疏向量
        sparse_vector = {int(idx): float(val) for idx, val in zip(query_embeddings["sparse"].indices, query_embeddings["sparse"].data)}
        # 7.14 创建稠密向量搜索请求
        ...
        sparse_req = AnnSearchRequest(data=[sparse_vector], anns_field="sparse_vector",
                                      param={"metric_type": "IP"}, limit=k, expr=filter_expr)

        # 7.16 Milvus混合搜索,结合稠密和稀疏向量
        milvus_results = self.client.hybrid_search(
            collection_name=config.MILVUS_COLLECTION_NAME,
            reqs=[dense_req, sparse_req],
            ranker=WeightedRanker(0.7, 0.3),
            limit=k,
            # filter=filter_expr, #search有filter参数
        ...
        # 7.26 按重排得分降序排序
        docs_with_scores.sort(key=lambda x: x.metadata['ranker_score'], reverse=True)
